...
class NavigateServer(base_node.BaseNode):
    ...
    async def _rcv_odom(self, msg: messages.Odometry) -> None:
        self._position = msg.position

    async def navigate(self, msg: messages.NavigateRequest) -> None:
        while (
            self._position is not None
            and abs(self._position - msg.position) > msg.tolerance
        ):
